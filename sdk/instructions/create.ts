...
async function createFractionIx(
  ...
  const ix = await program.methods
    .initializeFraction(fractionName, participants, botWallet)
    .accountsStrict({
      authority,
 ...
 */
async function createFraction(
  program: Program<Fraction>,
  input: CreatorFractionInputArgs,
  connection?: Connection,
  payer?: PublicKey
) {
  const { ix, fractionConfigPda, fractionVaultPda } = await createFractionIx(
    program,
    input
  );
